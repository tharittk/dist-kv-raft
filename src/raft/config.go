...
func (cfg *config) ingestSnap(i int, snapshot []byte, index int) string {
	...
	//fmt.Println(i, "has lastApplied[i] = lastIncludeIndex", lastIncludedIndex)
	return ""
}
...
func (cfg *config) nCommitted(index int) (int, interface{}) {
	...
	for i := 0; i < len(cfg.rafts); i++ {
		...
		//fmt.Println("in nCommitted index", index, "rf[id]", i, "cmd1, ok", cmd1, ok)
		cfg.mu.Unlock()
	...
	for time.Since(t0).Seconds() < 10 && cfg.checkFinished() == false {
		...
		for si := 0; si < cfg.n; si++ {
			...
			if rf != nil {
				...
				if ok {
					index = index1
					//fmt.Println("StartOK :: index1, ok", index1, ok)
					break
				}
			}
		}
		...
		if index != -1 {
			...
			for time.Since(t1).Seconds() < 2 {
				...
				//fmt.Println("check (nd, cmdq)", nd, cmd1, "want (expectSerer, cmd) ", expectedServers, cmd)
